...
例えば、任意の命題{$`P, Q`}に対して{$`P \land Q \to Q \land P`}が成り立つことの証明を、Lean 4では以下のように書くことができます。
```
...
ここでは仮定の項{$`\mathtt{h} \colon P ∧ Q`}から、その右の項{$`\mathtt{h.right} \colon Q`}と左の項{$`\mathtt{h.left} \colon P`}を取り出して、`And.intro`に適用することで{$`Q ∧ P`}型の項を生成しています。
