...
---
import Inserted from '../../../../components/Inserted'# 2024年3月20日## サイトをHonoXでリニューアル当サイトを[HonoX](https://github.com/honojs/honox)で作り直しました。当初は、テンプレートにMustacheを使い、R Markdownで記事を書いてHTML化する仕組みをRakuで自作していました。
しかしひとまずの完成を見たところで飽きてしまい、特に書くこともないし…と放置モードに入ってしまいました。
数年経って自作を維持していく気持ちもなくなっており、Hono🔥のフレームワークHonoXを試してみることにしました。
HonoはYAPC::Hiroshima 2024の前夜祭でファイルベースルーティングができるようになったというトークを聞いて使ってみたかったフレームワークでした。既存のR MarkdownファイルはMDXに変えて、MustacheテンプレートはHonoXで書き換えました。
...
こちらを`true`にすることで上記のような対処は必要なくなりました。</Inserted>使っていて気になったのは、`npm run dev`（`vite`）で`.html`を付けると404になってしまう点でした。
...
<Inserted date={{ year: 2025, month: 2, day: 24 }}>自己解決しました。[詳しくはこちらの日記をご覧ください。](/diary/2025/02/24.html)</Inserted>